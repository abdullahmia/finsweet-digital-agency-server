...
// order update email genarator
const OrderUpdateEmail = ({ id, name, status  }) => {
    let email = {
        body: {
            name: name,
            intro: `Your order status has been updated to <b>${status}</b>.`,
            action: {
                instructions: 'To see your order details, please click here:',
                button: {
                    color: '#22BC66', // Optional action button color
                    text: 'Order Details',
                    link: `${process.env.CLIENT_URL}/account/history/${id}`,
                }
            },
            outro: 'Need help, or have questions? Just reply to this email, we\'d love to help.'
        }
...
module.exports = { WelcomeEmail, ResetPasswordEmail, ServicePurchaseEmail, OrderUpdateEmail };